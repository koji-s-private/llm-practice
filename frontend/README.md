...
| コードブロックのシンタックスハイライト | [react-syntax-highlighter](https://github.com/react-syntax-highlighter/react-syntax-highlighter) + `@types/react-syntax-highlighter` | `src/components/chat/CodeBlock.tsx`。Prismの全言語データ（1MB超）を避けるため`prism-light`によく使う言語のみを個別`registerLanguage`し、`MarkdownContent.tsx`から`React.lazy`で遅延読み込みして本番バンドルを分割                    |
