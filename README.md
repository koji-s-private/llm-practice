...
| `frontend/` | Vite + React + TypeScript製の新フロントエンド（チャットUIまで実装済み。フロントエンド移行Step2/3, Issue #242, #256）。移行完了までStreamlit版（`app.py`）と並存する |
...
### フロントエンド基盤・チャットUI（Issue #242, #256）

[docs/frontend-tech-policy.md](docs/frontend-tech-policy.md)の移行計画Step2（基盤構築）・Step3（チャットUI）
として追加。`frontend/`配下にVite + React + TypeScriptでチャット画面を実装している。
...
| [TanStack Query](https://tanstack.com/query) | API通信のキャッシュ・再試行等を扱うデータ取得・状態管理ライブラリ | `frontend/src/main.tsx`（`QueryClientProvider`）。チャット送受信自体は`useChat`フックが直接`fetch`する（ストリーミング用途のためTanStack Queryは未使用） |
| [react-markdown](https://github.com/remarkjs/react-markdown) + [remark-gfm](https://github.com/remarkjs/remark-gfm) | チャット回答のMarkdown表示（表・取り消し線等のGFM記法を含む） | `frontend/src/components/markdown/Markdown.tsx` |
| [react-syntax-highlighter](https://github.com/react-syntax-highlighter/react-syntax-highlighter) | Markdown内のコードブロックのシンタックスハイライト | `frontend/src/components/markdown/Markdown.tsx` |
...
| [Vitest](https://vitest.dev/) + [React Testing Library](https://testing-library.com/react) | コンポーネント単体テスト | `frontend/src/App.test.tsx`、`frontend/src/components/chat/`、`frontend/src/lib/api.test.ts` |
| [Playwright](https://playwright.dev/)（Node版） | E2Eテスト（ローカル実行のみ、無料。バックエンドは`page.route`でモック） | `frontend/e2e/app.spec.ts`、`frontend/e2e/chat.spec.ts` |
