...
| `test_ingest_embedding_model_check.py` | `ingest.py` | `manifest.json`に記録した埋め込みモデル名と現在の`rag_chain.EMBEDDING_MODEL_NAME`の不一致検知・警告ログ（自動再構築はしない）のテスト |
...
| `test_app_feedback_buttons.py` | `app.py` | 回答フィードバックボタン（👍/👎）の表示・記録・重複押下防止・生成失敗時の非表示のテスト（`streamlit.testing.v1.AppTest` を使用） |
...
| `test_evaluate_model_accuracy.py` | `scripts/evaluate_model_accuracy.py` | RAGパイプライン全体の回答精度評価（キーワード含有率のスコアリング、正答率の集計、有料APIプロバイダの明示同意ガード）のテスト |
