...
| [ai-team.yml](ai-team.yml) | `ai-team-scheduler.yml`からの起動、または手動実行(Issue番号を指定) | 指定されたIssue1件について、PM役のエージェントが `coder`→`qa-engineer`→`reviewer` の順にサブエージェントへ実装・テスト・レビューを依頼する。reviewerは実際にGitHub PRレビューで`--comment`(LGTM相当)/`--request-changes`を投稿する(PR作成者(coder)とレビュアー(reviewer)が同じGitHub App ID(`claude[bot]`)のため、GitHub上の正式な`--approve`は自己承認扱いで拒否されるので使わない)。**LGTMが出てもマージは絶対に自動実行せず、必ず人間(koji)が手動で行う**(このプロジェクト固有の恒久方針) |
...
- **GitHub Actions**: GitHub上でコードのプッシュやIssueのコメントなど「何かが起きたとき」に、
  決められた処理（テスト実行・ビルド・今回のようなAIエージェントの起動など）を自動で走らせる仕組みです。
  このディレクトリのファイル（`.yml`）1つ1つが「ワークフロー」＝1つの自動処理の定義です。
- **`on:`（トリガー）**: そのワークフローが「いつ実行されるか」を指定する設定です。上の表の「起動タイミング」欄に対応します。
  - `schedule` + `cron`: 決まった時刻に定期実行する設定です（例: `cron: "0 1 * * *"` はUTCの1:00＝日本時間10:00）。
  - `workflow_dispatch`: GitHubの画面（Actionsタブ）やAPIから、人が手動でボタンを押して実行するためのトリガーです。
  - `issue_comment`: Issueに新しいコメントが投稿されたときに発火するトリガーです。
- **サブエージェント（`coder`/`qa-engineer`/`reviewer`）**: このワークフローの中でClaude Codeが呼び出す役割分担です。
  定義ファイルは [.claude/agents/](../../.claude/agents/) にあります。
- **Issue / PR（プルリクエスト）**: GitHubの機能で、Issueは「やること・不具合の報告」、
  PRは「実際のコード変更の提案（レビューを経てマージされる単位）」です。
