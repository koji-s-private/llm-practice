...
| [daily-health-check.yml](daily-health-check.yml) | 毎日 9:00 JST(cron) + 手動実行 | リポジトリ全体(`rag_chain.py`, `ingest.py`, `memory.py`, `app.py`, `setup.py`など)を能動的にスキャンし、バグ・改善点・検討事項・UX提案を優先度ラベル付きでIssue化する(広く浅い定期健診) |
