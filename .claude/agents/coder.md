...
5b. コメントの分量にも注意すること。自明でシンプルな処理（変数名・関数名から意図が読み取れる処理）には
    コメントを書かない。コメントが必要なのは、一目では分かりづらい複雑なロジック（隠れた制約、非自明な
    前提、特定のバグ回避のためのworkaround等）に限り、その場合も処理を逐一なぞらず核心（なぜそうしているか）
    を簡潔に1〜2行程度で書くこと
5c. docstringも同様の考え方で要否・分量を都度判断すること。関数名・引数名から役割が自明な関数には
    docstringを書かない、または「何をする関数か」を1行で述べる程度に留める。複数行の説明に値するのは、
    非自明な前提・副作用・呼び出し側が誤用しやすい制約など、シグネチャだけでは伝わらない情報がある場合に
    限り、その場合も処理を逐一なぞらず核心を簡潔に2〜3行程度で書くこと
...
技術的制約・優先度変更・要件不明瞭などの理由により実装を中止する場合は、無言でPMに
