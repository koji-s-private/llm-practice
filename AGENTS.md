...
- コード内コメントは日本語で記載する（新規追加・既存修正のいずれも）。ただし
  ...
  学習用スクリプトは対象外とする
- コード内コメント・docstring・テストの`describe`/`it`名など、コードに含まれるあらゆるテキストに
  「〇〇からの指示」「Issue #XXで発覚/指摘/対応」のような由来・経緯（いつ・誰の指示で・どの
  Issueで）を書き込むことは**絶対に禁止**。`(#123)`のようにIssue番号だけを括弧書きで添える
  パターンも同様に禁止（「Issue」という単語が無くても、末尾に番号だけを添える形はすべて対象）。
  由来・経緯はコミット履歴やPR本文（`Closes #XX` 等）を辿れば十分な情報であり、コード側に残すのは
  リファクタリングやIssueクローズ後に文脈が失われて陳腐化するだけの純粋なノイズ。reviewerは
  このパターンを見つけたら他の観点の良し悪しに関わらず必ず要修正判定にする（機械的なreject対象）
- コメント・docstringは「何のためにこの処理があるか」「パフォーマンスやセキュリティなど、なぜ
  この処理をここに書く必要があるか」だけを端的に書く。「〇〇という対応をした結果△△という問題が
  起こるため」のように背景シナリオを丁寧になぞる説明は書かない。背景説明がどうしても必要な場合も、
  結論（何のため・なぜここか）を先に一言で述べ、背景は補足として最小限に留める
- コメントは分量そのものにも注意する。自明でシンプルな処理（変数名・関数名から意図が読み取れる
  処理）にはコメントを書かない。コメントが必要なのは、一目では分かりづらい複雑なロジック
  （隠れた制約、非自明な前提、特定のバグ回避のためのworkaround等）に限る。その場合も、処理を
  逐一なぞる説明ではなく、核心（なぜそうしているか）を簡潔に1〜2行程度で書く
- docstringも同様の考え方で要否・分量を判断する。関数名・引数名から役割が自明な関数には
  docstringを書かない、または「何をする関数か」を1行で述べる程度に留める。複数行の説明に
  値するのは、非自明な前提・副作用・呼び出し側が誤用しやすい制約など、シグネチャだけでは
  伝わらない情報がある場合に限り、その場合も処理を逐一なぞらず核心（なぜそうしているか・
  何に注意すべきか）を簡潔に2〜3行程度で書く。実装時にどちらに該当するかはcoder自身が
  都度判断する
...
## スコープ外の発見事項の扱い
- coder / qa-engineer / reviewer / designer が作業中に気づいた問題（バグ、デグレ、技術的負債、改善点）は、
  「今回のPRでのコード修正・機能追加が原因で発生した（または発生しうる）もの」か「今回の変更とは
  全く関係のない既存コードの問題にたまたま気づいたもの」かで扱いを分ける
  - 前者（このPRの修正・追加に付随して新たに生じた不具合・デグレ）は、スコープ外扱いにせず同じPR内で
    coderが修正し、qa-engineerが検証する
  - 後者（このPRの変更内容とは無関係な既存コードの問題）のみ、その場では直さずPMへの報告に
    「スコープ外の発見事項」として含める
- PMは後者についてのみ新しいIssueとして作成し、`found-in-review` ラベルを付ける（Statusは `Todo`。既存Issueとの重複がないか事前に確認すること）
